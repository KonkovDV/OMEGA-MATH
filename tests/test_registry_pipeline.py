...
class RegistryPipelineTests(unittest.TestCase):
    ...
    def test_resolve_snapshot_date_prefers_existing_index_in_shallow_clone(self) -> None:
        _write_yaml(
            self.repo_root / "registry" / "domains" / "number-theory.yaml",
            {
                "problems": [
        ...
        with self._patch_generate_index(), patch.object(generate_index, "is_shallow_repository", return_value=True), patch.object(
            generate_index,
            "subprocess",
            MagicMock(run=MagicMock(return_value=MagicMock(stdout="2026-04-16\n", returncode=0))),
        ):
            snapshot = generate_index.resolve_snapshot_date()
